...
router.post('/', requireAuth, upload.array('images', 4), async (req, res) => {
  ...
  db.prepare('INSERT INTO posts (id,user_id,category_id,type,title,body,location,email_token) VALUES (?,?,?,?,?,?,?,?)')
    .run(id, req.session.userId, category_id || null, type, title.trim(), body.trim(), location || '', emailToken);
