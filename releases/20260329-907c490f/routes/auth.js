...
router.post('/register', async (req, res) => {
  ...
  db.prepare('INSERT INTO users (id,email,password,name,verify_token) VALUES (?,?,?,?,?)').run(id, email.toLowerCase(), hash, name, token);
...
router.post('/login', async (req, res) => {
  ...
  res.json({ ok: true, user: { id: user.id, name: user.name, email: user.email, avatar: user.avatar, role: user.role } });
});

router.post('/logout', (req, res) => { req.session.destroy(() => res.json({ ok: true })); });

router.get('/me', requireAuth, (req, res) => {
  const user = db.prepare('SELECT id,email,name,bio,avatar,location,phone,verified,role,created_at FROM users WHERE id=?').get(req.session.userId);
