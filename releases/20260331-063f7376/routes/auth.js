...
router.post('/register', async (req, res) => {
  ...
  if (name.trim().length < 2 || name.trim().length > 60) return res.status(400).json({ error: 'Navn må være mellom 2 og 60 tegn' });
  if (password.length < 8) return res.status(400).json({ error: 'Passord må være minst 8 tegn' });
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return res.status(400).json({ error: 'Ugyldig e-postadresse' });
  if (password.length < 8) return res.status(400).json({ error: 'Passord må være minst 8 tegn' });
