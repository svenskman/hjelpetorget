...
function requireValidId(req, res, next) {
  const id = req.params.id || req.params.postId || req.params.userId;
  if (id && id !== '__breakglass__' && !isValidUUID(id)) {
...
app.get('/api/admin/categories', requireAdmin, (req, res) => {
  ...
  for (const c of cats) {
    c.count = db.prepare("SELECT COUNT(*) as n FROM posts WHERE category_id=? AND status='open'").get(c.id).n;
    if (c.parent_id) {
      const parent = cats.find(p => p.id === c.parent_id);
      if (parent) { c.icon = parent.icon; c.color = parent.color; }
    }
  }
  res.json(cats);
});

// Public: categories as tree (root → sub → speciality)
app.get('/api/categories/tree', (req, res) => {
  ...
  for (const c of cats) {
    if (c.parent_id && byId[c.parent_id]) {
      // Inherit icon/color from root
      const root = c.parent_id && byId[c.parent_id] && byId[c.parent_id].parent_id
        ? byId[byId[c.parent_id].parent_id] : byId[c.parent_id];
      byId[c.id].icon = root ? root.icon : c.icon;
      byId[c.id].color = root ? root.color : c.color;
...
app.post('/api/admin/categories', requireAdmin, (req, res) => {
  const { name, slug, icon, color, parent_id } = req.body;
  if (!name || !slug) return res.status(400).json({ error: 'Navn og slug er påkrevd' });
  if (!parent_id && !icon) return res.status(400).json({ error: 'Ikon er påkrevd for hovedkategorier' });
  const pId = parent_id ? parseInt(parent_id) : null;
  const parent = pId ? db.prepare('SELECT * FROM categories WHERE id=?').get(pId) : null;
  const maxOrder = db.prepare('SELECT COALESCE(MAX(sort_order),0) as m FROM categories WHERE ' + (pId ? 'parent_id=?' : 'parent_id IS NULL')).get(...(pId ? [pId] : [])).m || 0;
  try {
    db.prepare('INSERT INTO categories (parent_id,slug,name,icon,color,sort_order) VALUES (?,?,?,?,?,?)')
      .run(pId, slug, name, parent ? parent.icon : (icon || '📦'), parent ? parent.color : (color || '#AAAAAA'), maxOrder + 1);
    res.json({ ok: true });
...
app.put('/api/admin/categories/:id', requireAdmin, (req, res) => {
  ...
  if (!cat) return res.status(404).json({ error: 'Ikke funnet' });
  if (!cat.parent_id) {
    db.prepare('UPDATE categories SET name=?,icon=?,color=? WHERE id=?').run(name, icon, color || '#AAAAAA', req.params.id);
    // Cascade icon/color to children and grandchildren
    const updateChildren = (parentId, ico, col) => {
      const kids = db.prepare('SELECT id FROM categories WHERE parent_id=?').all(parentId);
      for (const k of kids) { db.prepare('UPDATE categories SET icon=?,color=? WHERE id=?').run(ico, col, k.id); updateChildren(k.id, ico, col); }
    };
    updateChildren(req.params.id, icon, color || '#AAAAAA');
  } else {
...
app.delete('/api/admin/categories/:id', requireAdmin, (req, res) => {
  // CASCADE in DB handles children
...
app.get('/api/locations', (req, res) => {
  const rows = db.prepare('SELECT * FROM locations ORDER BY sort_order, fylke, kommune').all();
  const tree = {};
  for (const r of rows) {
    if (!tree[r.fylke]) tree[r.fylke] = { id: r.id, fylke: r.fylke, kommuner: [] };
    if (r.kommune) tree[r.fylke].kommuner.push({ id: r.id, name: r.kommune });
  }
  res.json(Object.values(tree));
});

// Admin: add fylke
app.post('/api/admin/locations/fylke', requireAdmin, (req, res) => {
  const { fylke } = req.body;
  if (!fylke || !fylke.trim()) return res.status(400).json({ error: 'Fylkenavn er påkrevd' });
  const maxOrder = db.prepare('SELECT MAX(sort_order) as m FROM locations').get().m || 0;
  try {
    db.prepare('INSERT INTO locations (fylke, kommune, sort_order) VALUES (?,NULL,?)').run(fylke.trim(), maxOrder + 1);
    res.json({ ok: true });
  } catch { res.status(409).json({ error: 'Fylket finnes allerede' }); }
});

// Admin: add kommune to fylke
app.post('/api/admin/locations/kommune', requireAdmin, (req, res) => {
  const { fylke, kommune } = req.body;
  if (!fylke || !kommune) return res.status(400).json({ error: 'Fylke og kommune er påkrevd' });
  const maxOrder = db.prepare('SELECT MAX(sort_order) as m FROM locations WHERE fylke=?').get(fylke).m || 0;
  try {
    db.prepare('INSERT INTO locations (fylke, kommune, sort_order) VALUES (?,?,?)').run(fylke.trim(), kommune.trim(), maxOrder + 1);
    res.json({ ok: true });
  } catch { res.status(409).json({ error: 'Kommunen finnes allerede i dette fylket' }); }
});

// Admin: delete location (fylke or kommune)
app.delete('/api/admin/locations/:id', requireAdmin, (req, res) => {
  ...
  if (!loc) return res.status(404).json({ error: 'Ikke funnet' });
  if (!loc.kommune) {
    // Deleting a fylke - delete all its kommuner too
    db.prepare('DELETE FROM locations WHERE fylke=?').run(loc.fylke);
  } else {
...
// Report a user
const reportLimit = rateLimit({ windowMs: 60 * 60 * 1000, max: 10, message: { error: 'For mange rapporter. Prøv igjen om en time.' } });
app.post('/api/report/user/:id', requireAuth, reportLimit, requireValidId, (req, res) => {
...
// ── Quick approve via e-post token (no admin session needed) ─────────────────
app.get('/api/quick-approve/:token', (req, res) => {
  const { token } = req.params;
  if (!token || token.length < 32) return res.status(400).send('Ugyldig lenke');

  const user = db.prepare('SELECT * FROM users WHERE verify_token=? AND trust_status IN (?,?)').get(token, 'pending', 'vouched');
  if (!user) return res.status(404).send('<h2>Lenken er ugyldig eller brukeren er allerede behandlet.</h2>');

  // Check that requester is an admin (via session) OR has a valid admin quick-token
  const adminToken = req.query.admin_token;
  const validAdminToken = db.prepare("SELECT id FROM admin_quick_tokens WHERE token=? AND expires_at > unixepoch()").get(adminToken);
  const isAdminSession = req.session && req.session.role === 'admin';

  if (!isAdminSession && !validAdminToken) {
    return res.redirect('/?quick_approve_token=' + token);
  }

  db.prepare("UPDATE users SET trust_status='active', approved_by=?, verified=1 WHERE id=?").run('quick-approve', user.id);
  notify(user.id, 'account_approved', { approved_by: 'admin' });

  // Clean up token if used
  if (validAdminToken) db.prepare('DELETE FROM admin_quick_tokens WHERE token=?').run(adminToken);

  res.send('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Godkjent</title></head><body style="font-family:sans-serif;max-width:480px;margin:80px auto;text-align:center">' +
    '<h2 style="color:#6AAF7C">✅ Bruker godkjent!</h2>' +
    '<p><strong>' + user.name + '</strong> (' + user.email + ') er nå aktiv på Hjelpetorget.</p>' +
    '<p><a href="/">Gå til Hjelpetorget</a></p></body></html>');
});

// Admin: get pending count (lightweight, for topbar badge — works without full admin session)
app.get('/api/pending-count', requireAuth, (req, res) => {
  const user = db.prepare('SELECT role FROM users WHERE id=?').get(req.session.userId);
  if (!user || user.role !== 'admin') return res.json({ count: 0 });
  const count = db.prepare("SELECT COUNT(*) as n FROM users WHERE trust_status IN ('pending','vouched')").get().n;
  res.json({ count });
});

// Admin: send quick-approve links for pending users (requires active admin session)
app.post('/api/admin/send-approve-links', requireAdmin, async (req, res) => {
  const pending = db.prepare("SELECT * FROM users WHERE trust_status IN ('pending','vouched') ORDER BY created_at DESC").all();
  if (!pending.length) return res.json({ ok: true, message: 'Ingen ventende brukere' });

  const { sendMail } = require('./utils');
  const admin = db.prepare('SELECT email,name FROM users WHERE id=?').get(req.session.userId);
  const sent = [];

  for (const u of pending) {
    // Send email to admin with one-click approve link
    const approveUrl = 'https://' + BASE + '/api/quick-approve/' + u.verify_token;
    await sendMail({
      to: admin.email,
      subject: '⏳ Ventende bruker: ' + u.name,
      html: '<div style="font-family:sans-serif;max-width:520px">' +
        '<h2 style="color:#C4613B">⏳ Ny bruker venter på godkjenning</h2>' +
        '<p><strong>' + u.name + '</strong><br>' + u.email + '</p>' +
        '<p>Status: <strong>' + u.trust_status + '</strong>' +
        (u.vouched_by ? '<br>Gått god for av: ' + (db.prepare('SELECT name FROM users WHERE id=?').get(u.vouched_by)||{name:'ukjent'}).name : '') +
        '</p>' +
        '<p style="margin-top:20px">' +
        '<a href="' + approveUrl + '" style="background:#6AAF7C;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:600">✅ Godkjenn ' + u.name + '</a>' +
        '</p>' +
        '<p style="font-size:.85rem;color:#888;margin-top:16px">Du må være innlogget som admin for at lenken skal fungere.</p>' +
        '</div>',
    }).catch(e => console.warn('[approve-mail]', e.message));
    sent.push(u.name);
  }

  res.json({ ok: true, sent, message: 'E-post sendt for ' + sent.length + ' bruker(e)' });
});

// ── Terms of service API ─────────────────────────────────────────────────────

// Get current terms (public)
app.get('/api/terms/current', (req, res) => {
  const t = db.prepare('SELECT * FROM terms_versions WHERE is_current=1 ORDER BY id DESC LIMIT 1').get();
  res.json(t || null);
});

// Get all versions (admin)
app.get('/api/terms/versions', requireAdmin, (req, res) => {
  res.json(db.prepare('SELECT * FROM terms_versions ORDER BY id DESC').all());
});

// Publish new version (admin) — sends email + notification to all users
app.post('/api/terms/publish', requireAdmin, async (req, res) => {
  const { version, content, summary } = req.body;
  if (!version || !content) return res.status(400).json({ error: 'Versjon og innhold er påkrevd' });
  if (!summary) return res.status(400).json({ error: 'Endringssammendrag er påkrevd' });

  const existing = db.prepare('SELECT id FROM terms_versions WHERE version=?').get(version);
  if (existing) return res.status(409).json({ error: 'Versjon ' + version + ' finnes allerede' });

  const admin = db.prepare('SELECT name,email FROM users WHERE id=?').get(req.session.userId);

  // Deactivate old current version
  db.prepare('UPDATE terms_versions SET is_current=0').run();

  // Insert new version
  db.prepare('INSERT INTO terms_versions (version,content,summary,published_by,is_current) VALUES (?,?,?,?,1)')
    .run(version, content, summary, req.session.userId);

  adminLog(req.session.userId, admin.name, 'TERMS_PUBLISHED', version, summary);

  // Notify all active users
  const users = db.prepare("SELECT id,email,name FROM users WHERE trust_status IN ('active','verified') AND id != ?").all(req.session.userId);
  const { sendMail } = require('./utils');
  const BASE_URL = 'https://' + BASE;
  let sent = 0;

  for (const u of users) {
    // Internal notification
    notify(u.id, 'terms_updated', {
      version,
      summary,
    ...
    await sendMail({
      to: u.email,
      subject: '📋 Hjelpetorget – Oppdatert brukeravtale v' + version,
      html: '<div style="font-family:sans-serif;max-width:560px;color:#2C1810">' +
        '<div style="background:#C4613B;padding:16px 20px;border-radius:8px 8px 0 0">' +
        '<h2 style="margin:0;color:#fff;font-size:1rem">📋 Hjelpetorget – Oppdatert brukeravtale</h2></div>' +
        '<div style="background:#FAF6F0;padding:20px;border-radius:0 0 8px 8px;border:1px solid #DDD5C8">' +
        '<p>Hei ' + u.name + ',</p>' +
        '<p>Hjelpetorgets brukeravtale er oppdatert til <strong>versjon ' + version + '</strong>.</p>' +
        '<div style="background:#fff;border-left:4px solid #C4613B;padding:12px 16px;margin:16px 0;border-radius:0 6px 6px 0">' +
        '<p style="margin:0;font-weight:600;color:#C4613B">Hva er endret:</p>' +
        '<p style="margin:8px 0 0">' + summary + '</p>' +
        '</div>' +
        '<p>Du kan lese den fullstendige avtalen ved å logge inn på <a href="' + BASE_URL + '">' + BASE_URL + '</a> og klikke på brukeravtalen ved registrering eller i innstillinger.</p>' +
        '<p style="color:#888;font-size:.85rem">Fortsatt bruk av Hjelpetorget etter denne datoen anses som aksept av de nye vilkårene.</p>' +
        '</div></div>',
    }).catch(() => {});
    sent++;
  }

  res.json({ ok: true, version, sent, message: 'Versjon ' + version + ' publisert og sendt til ' + sent + ' brukere.' });
});

// Accept terms (user)
app.post('/api/terms/accept', requireAuth, (req, res) => {
  const current = db.prepare('SELECT version FROM terms_versions WHERE is_current=1').get();
  if (!current) return res.status(404).json({ error: 'Ingen aktiv brukeravtale funnet' });
  try {
    db.prepare('INSERT OR REPLACE INTO user_terms_accepted (user_id,terms_version) VALUES (?,?)').run(req.session.userId, current.version);
    db.prepare('UPDATE users SET terms_version=? WHERE id=?').run(current.version, req.session.userId);
  } catch {}
  res.json({ ok: true, version: current.version });
});

// Check if user needs to re-accept (after terms update)
app.get('/api/terms/check', requireAuth, (req, res) => {
  const current = db.prepare('SELECT version FROM terms_versions WHERE is_current=1').get();
  if (!current) return res.json({ upToDate: true });
  const user = db.prepare('SELECT terms_version FROM users WHERE id=?').get(req.session.userId);
  res.json({ upToDate: user && user.terms_version === current.version, currentVersion: current.version, userVersion: user && user.terms_version });
});
...
app.post('/api/breakglass', (req, res) => {
  ...
  // Use a reserved admin user ID that cannot match real users (UUIDs are 36 chars)
  req.session.userId = '__breakglass__';
  req.session.role   = 'admin';
  res.json({ ok: true });
});
...
// ── Lab: clean install download ──────────────────────────────────────────────
app.get('/api/lab/clean-install', requireAdmin, (req, res) => {
  if (!IS_LAB) return res.status(400).json({ error: 'Kun tilgjengelig på lab-instansen' });

  const FILES = [
    'server.js','db.js','utils.js','session-store.js','content-filter.js',
    'package.json','routes/auth.js','routes/posts.js','routes/profile.js',
    'routes/messages.js','public/index.html'
  ];

  const README = '# Hjelpetorget\n\nEn gratis nabohjelp-plattform der naboer hjelper naboer.\nTilby din tid eller be om hjelp — helt uforpliktende.\n\n## Stack\n\n- **Runtime:** Node.js 20 (Alpine)\n- **Framework:** Express\n- **Database:** SQLite (better-sqlite3)\n- **E-post:** Nodemailer (SMTP)\n- **Bilder:** Multer + Sharp\n- **Auth:** express-session + bcrypt\n\n---\n\n## Hurtigstart\n\n```bash\nnpm install\nnode server.js\n```\n\nÅpne http://localhost:3000\n\n---\n\n## Miljøvariabler\n\n### Påkrevd\n\n| Variabel | Beskrivelse | Eksempel |\n|---|---|---|\n| `SESSION_SECRET` | Hemmelig nøkkel for sesjonssignering — minst 32 tilfeldige tegn | `openssl rand -hex 32` |\n| `DATA_DIR` | Mappe for SQLite-database og opplastinger | `/data` |\n\n### Domener\n\n| Variabel | Beskrivelse | Eksempel |\n|---|---|---|\n| `BASE_DOMAIN` | Hoveddomenets hostname | `hjelpetorget.no` |\n| `ADMIN_DOMAIN` | Admin-subdomenets hostname | `admin.hjelpetorget.no` |\n\n### E-post (SMTP)\n\n| Variabel | Beskrivelse | Eksempel |\n|---|---|---|\n| `SMTP_HOST` | SMTP-server | `mail.runbox.com` |\n| `SMTP_PORT` | SMTP-port | `587` |\n| `SMTP_USER` | SMTP-brukernavn | `post@hjelpetorget.no` |\n| `SMTP_PASS` | SMTP-passord | `hemmelig` |\n| `SMTP_FROM` | Avsenderadresse | `Hjelpetorget <post@hjelpetorget.no>` |\n\n### GitHub-integrasjon (valgfri)\n\nBrukes for automatisk deployment og versjonshåndtering mellom lab og prod.\n\n| Variabel | Beskrivelse | Eksempel |\n|---|---|---|\n| `GITHUB_REPO` | GitHub-repo i format `bruker/repo` | `mittbrukernavn/hjelpetorget` |\n| `GITHUB_TOKEN` | Personal Access Token med `repo`-scope | `ghp_xxx...` |\n\nOpprett token: GitHub → Settings → Developer settings → Personal access tokens\n\n### Lab/Prod-modus\n\n| Variabel | Beskrivelse | Verdi |\n|---|---|---|\n| `IS_LAB` | Kjør i lab-modus (full admin, filoppasting) | `true` / `false` |\n\n### Portainer-integrasjon (valgfri)\n\nBrukes for å styre lab-containeren fra prod-admin.\n\n| Variabel | Beskrivelse | Eksempel |\n|---|---|---|\n| `PORTAINER_URL` | Portainer API-URL | `https://192.168.1.10:9443` |\n| `PORTAINER_TOKEN` | Portainer API-token | `ptr_xxx...` |\n| `PORTAINER_ENV_ID` | Portainer environment ID | `1` |\n| `PORTAINER_LAB_CONTAINER` | Navn på lab-containeren | `hjelpetorget-lab` |\n\n### Nødtilgang (Breakglass)\n\nTre separate nødtokener som gir admin-tilgang uten innlogging.\nSett minst ett — oppbevar sikkert og separat fra kodebasen.\n\n| Variabel | Beskrivelse |\n|---|---|\n| `BREAKGLASS_1` | Nødtoken 1 |\n| `BREAKGLASS_2` | Nødtoken 2 |\n| `BREAKGLASS_3` | Nødtoken 3 |\n\n### IMAP (valgfri, for e-posttriggere)\n\n| Variabel | Beskrivelse | Eksempel |\n|---|---|---|\n| `IMAP_PORT` | IMAP-port | `993` |\n| `IMAP_FOLDER` | IMAP-mappe | `INBOX` |\n| `IMAP_POLL_INTERVAL` | Polling-intervall i ms | `300000` |\n\n---\n\n## Docker Compose (anbefalt oppsett)\n\n```yaml\nservices:\n  hjelpetorget:\n    image: node:20-alpine\n    container_name: hjelpetorget\n    working_dir: /app\n    volumes:\n      - ./app:/app\n      - ./data:/data\n    command: sh -c "[ -d node_modules ] || npm install --omit=dev && node server.js"\n    environment:\n      - NODE_ENV=production\n      - DATA_DIR=/data\n      - BASE_DOMAIN=hjelpetorget.no\n      - ADMIN_DOMAIN=admin.hjelpetorget.no\n      - SESSION_SECRET=BYTT_MED_TILFELDIG_STRENG\n      - SMTP_HOST=din.smtp.server\n      - SMTP_PORT=587\n      - SMTP_USER=post@ditt-domene.no\n      - SMTP_PASS=ditt-passord\n      - SMTP_FROM=Hjelpetorget <post@ditt-domene.no>\n      - IS_LAB=false\n      - BREAKGLASS_1=BYTT_MED_HEMMELIG_TOKEN\n    restart: unless-stopped\n    ports:\n      - "3000:3000"\n```\n\n### Med Traefik (reverse proxy + SSL)\n\n```yaml\n    labels:\n      - "traefik.enable=true"\n      - "traefik.http.routers.hjelpetorget.rule=Host(`hjelpetorget.no`) || Host(`www.hjelpetorget.no`) || Host(`admin.hjelpetorget.no`)"\n      - "traefik.http.routers.hjelpetorget.entrypoints=web-secure"\n      - "traefik.http.routers.hjelpetorget.tls=true"\n      - "traefik.http.services.hjelpetorget-svc.loadbalancer.server.port=3000"\n```\n\n---\n\n## Første oppstart\n\n1. Server starter og oppretter databasen automatisk\n2. Gå til http://localhost:3000 og registrer deg\n3. Sett admin-rolle via terminalen:\n\n```bash\ndocker exec hjelpetorget node -e "\nconst db = require(\'./db\');\ndb.prepare(\\"UPDATE users SET role=\'admin\', trust_status=\'active\', verified=1 WHERE email=\'din@epost.no\'\\").run();\nconsole.log(\'Admin satt!\');\n"\n```\n\n4. Logg inn og gå til Admin-panelet\n\n---\n\n## Lab/Prod-oppsett (valgfritt)\n\nKjør to instanser — én lab for testing og én prod for produksjon.\nLab publiserer kode til GitHub, prod henter automatisk.\n\n```\nlab.domene.no    → IS_LAB=true,  GITHUB_TOKEN=xxx, GITHUB_REPO=bruker/repo\nprod.domene.no   → IS_LAB=false, GITHUB_TOKEN=xxx, GITHUB_REPO=bruker/repo\n```\n\nSe Admin → 📖 Manual for fullstendig veiledning.\n\n---\n\n## Mappestruktur\n\n```\n/app\n├── server.js          Hoved-Express-app og alle API-ruter\n├── db.js              SQLite-schema, migrasjoner og seed-data\n├── utils.js           Multer, Sharp, auth-middleware, Nodemailer\n├── session-store.js   SQLite-basert session store\n├── content-filter.js  Automatisk sensurfilter\n├── package.json\n├── routes/\n│   ├── auth.js        Innlogging, registrering, passordreini\n│   ├── posts.js       Annonser og kategorier\n│   ├── profile.js     Brukerprofiler og vurderinger\n│   └── messages.js    Meldinger og samtaler\n└── public/\n    └── index.html     Komplett SPA-frontend\n```\n\n---\n\n## Lisens\n\nMIT — fri til bruk, modifisering og distribusjon.\n';

  // Build ZIP in memory using our existing extractZip logic in reverse
  // Use a simple ZIP builder
  function buildZip(entries) {
    ...
    for (const [name, data] of entries) {
      const nameBuf  = Buffer.from('hjelpetorget/' + name);
      ...
      cdEntry.writeUInt32LE(dataBuf.length, 20);
  ...
  try {
    ...
    // Add source files — strip any existing user data references
    ...
    // Add routes subdirectory marker (empty)
    const zipBuf = buildZip(entries);

    res.setHeader('Content-Type', 'application/zip');
    res.setHeader('Content-Disposition', 'attachment; filename="hjelpetorget-clean-install.zip"');
    res.setHeader('Content-Length', zipBuf.length);
    res.end(zipBuf);

    console.log('[clean-install] Downloaded by', req.session.userId, '- ' + entries.length + ' files, ' + zipBuf.length + ' bytes');
  } catch(e) {
